...
# Script de test SIH avec authentification simplifiée
# Utilise l'authentification de session Laravel standard
...
LOG_FILE="$PROJECT_DIR/test_simple_auth_results.log"
OPERATIONS_LOG="$PROJECT_DIR/simple_auth_operations.log"
ERROR_COUNT=0
SUCCESS_COUNT=0
...
COOKIE_FILE="$PROJECT_DIR/auth_cookies.txt"
...
authenticate() {
    ...
    # 2. Extraire le token CSRF de différentes manières possibles
    csrf_token=""
    
    # Méthode 1: depuis l'attribut value
    csrf_token=$(echo "$login_page" | grep -o 'name="_token"[^>]*value="[^"]*' | sed 's/.*value="\([^"]*\)".*/\1/' | head -1)
    
    # Méthode 2: depuis l'attribut content (meta tag)
    if [ -z "$csrf_token" ]; then
        csrf_token=$(echo "$login_page" | grep -o 'name="csrf-token"[^>]*content="[^"]*' | sed 's/.*content="\([^"]*\)".*/\1/' | head -1)
    fi
    
    # Méthode 3: depuis un champ caché
    if [ -z "$csrf_token" ]; then
        csrf_token=$(echo "$login_page" | grep -o '<input[^>]*name="_token"[^>]*value="[^"]*' | sed 's/.*value="\([^"]*\)".*/\1/' | head -1)
    fi
    
    if [ -z "$csrf_token" ]; then
        log_warning "Token CSRF non trouvé, tentative sans token"
    fi
    ...
    if [ "$http_code" = "200" ] || [ "$http_code" = "302" ]; then
        ...
        # 5. Vérifier qu'on est bien authentifié en testant une page protégée
        test_response=$(curl -s -b "$COOKIE_FILE" "http://localhost:$BACKEND_PORT/dashboard" -w "%{http_code}" | tail -c 3)
        
        if [ "$test_response" = "200" ]; then
            log_success "Session authentifiée confirmée!"
            return 0
        else
            log_warning "Authentification OK mais session non confirmée (Status: $test_response)"
            return 1
        fi
    else
...
# Démarrage du script
echo "=========================================="
echo "🔐 SCRIPT DE TEST SIH AUTHENTIFIÉ (SIMPLE)"
echo "=========================================="
echo "Début: $(date)"
...
echo "Test SIH Simple Auth - $(date)" > "$LOG_FILE"
echo "==========================================" >> "$LOG_FILE"
echo "Opérations SIH Simple Auth - $(date)" > "$OPERATIONS_LOG"
echo "==========================================" >> "$OPERATIONS_LOG"
...
if ! authenticate; then
    log_error "Impossible de s'authentifier, tests sans authentification"
    echo "Continuation avec les tests publics..."
fi
